...
void test1() {
    ...
    std::list<int> l4(10, 100); // 10 elements with value 100
    display(l4);
}
...
void test3() {
    ...
    persons.resize(5); // 5 default Person objects
    display(persons);
}
...
void test4() {
    ...
    l.insert(it, l2.begin(), l2.end()); // insert l2 before 5
    display(l);

    std::advance(it, -4); // move iterator back 4 positions
    std::cout << "Erasing: " << *it << std::endl;
    it = l.erase(it); // erase the element at iterator and return next element
    display(l);
}
...
void test5() {
    ...
    std::cout << "Enter the age of the stooge: ";
    std::cin >> age;
    
    stooges.emplace_back(name, age); // add to the end of the list
    display(stooges);

    // Insert Frank before Moe
    auto it = std::find(stooges.begin(), stooges.end(), Person{"Moe", 25});
    if (it != stooges.end()) {
        stooges.emplace(it, "Frank", 18); // add before Moe 
    }
    display(stooges);
}
